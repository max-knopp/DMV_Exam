  ...
  mutate( 
    name = str_trim(
      ...
      experience_level = case_when(
        experience_years <= 2 ~ "junior",
        experience_years <= 5 ~ "mid",
...
glimpse(merged) 
#write_csv(merged, "merged_output_v8.csv")
...
merged %>%
  summarise(
    min_fare_per_km = min(gross_fare_usd / distance_km, na.rm = TRUE),
    max_fare_per_km  = max(gross_fare_usd / distance_km, na.rm = TRUE),
    mean_fare_per_km = mean(gross_fare_usd / distance_km, na.rm = TRUE)
  )

# fares vs duration
ggplot(merged, aes(x = duration_min, y = net_fare_usd, color = ride_category)) +
...
# fares vs duration with cities
ggplot(merged, aes(x = duration_min, y = net_fare_usd, color = city_ride)) +
  geom_point(alpha = 0.4) +
  labs(
    title = "Relationship between Ride Duration and Fare by City",
    x = "Duration (minutes)",
    y = "Fare (USD)",
    color = "City"
  ) + 
...
# fares vs duration with weekend
ggplot(merged, aes(x = duration_min, y = net_fare_usd, color =is_weekend)) +
  geom_point(alpha = 0.4) +
  labs(
...
#------------discount-------------------------------------------------
#count rides where discount code was redeemed
merged %>%
  summarise(
    no_discount_code = sum(discount_percentage == 0, na.rm = TRUE),
    percentage_no_discount_code = no_discount_code / sum(ride_id != ''),
    percentage_discount_code = 1- percentage_no_discount_code
  )
...
#mean effective discount  in different cities
merged %>%
  group_by(city_ride) %>%
  summarise(
...
# usage of different discount codes city comparison
merged %>%
  group_by(city_ride) %>%
  summarise(
    total_rides          = n(),
...
#trips in several cities in one day
merged %>%
  group_by(driver_id, date) %>%
  summarise(
    n_cities = n_distinct(city_ride),
    .groups = "drop"
  ) %>%
  filter(n_cities > 1)

#count day rider pairs (more than one city per day)
merged %>%
  group_by(driver_id, date) %>%
  summarise(
    n_cities = n_distinct(city_ride),
...
#riders + cities, when they were active in more than one city during one day
merged %>%
  group_by(driver_id, date) %>%
  filter(n_distinct(city_ride) > 1) %>%
  summarise(
    cities = paste(sort(unique(city_ride)), collapse = ", "),
    .groups = "drop"
  )
print(n=50)

#count riders that drive in more than one city in a day at least once
merged %>%
  group_by(driver_id, date) %>%
  summarise(
    n_cities = n_distinct(city_ride),
...
# control: at least once two or more rides in one day
merged %>%
  group_by(driver_id, date) %>%
  summarise(
    n_rides = n(),
...
#old experience levels thresholds (we switched to hard coded limits)
